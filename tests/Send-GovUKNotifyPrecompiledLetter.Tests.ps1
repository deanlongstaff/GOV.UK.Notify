...
Describe 'Send-GovUKNotifyPrecompiledLetter' {
    ...
    BeforeEach {
        Mock -ModuleName GOV.UK.Notify Invoke-GovUKNotifyApi { @{ id = 'n1'; reference = 'ref'; postage = 'second' } }
    }
    ...
    It 'includes postage when supplied' {
        ...
        Should -Invoke -ModuleName GOV.UK.Notify Invoke-GovUKNotifyApi -Times 1 -ParameterFilter { $Body.postage -eq 'first' }
    }
    ...
    It 'reads a file from -Path and base64-encodes it' {
        ...
        Should -Invoke -ModuleName GOV.UK.Notify Invoke-GovUKNotifyApi -Times 1 -ParameterFilter { $Body.content -eq $Expected }
    }
}
